 ...
 func Graph() *graph.Graph {
 	return graph.New().
 		// swi
 		Prov("swi").Conn("mun", c.Land).Conn("tyr", c.Land).Conn("pie", c.Land).Conn("bur", c.Land).Flag(c.Land).SC(c.Neutral).
    // nat
 		...
 		// stp
 		Prov("stp").Conn("fin", c.Land).Conn("nwy", c.Land).Conn("mos", c.Land).Conn("lvn", c.Land).Flag(c.Land).Conn("bar", c.Sea).Conn("bot", c.Sea).SC(c.Poland-Lithuania).
 		// mos
 		Prov("mos").Conn("stp", c.Land).Conn("sev", c.Land).Conn("ukr", c.Land).Conn("war", c.Land).Conn("lvn", c.Land).Flag(c.Land).SC(c.Neutral).
 		// sev
 		Prov("sev").Conn("ukr", c.Land).Conn("mos", c.Land).Conn("arm", c.Coast...).Conn("bla", c.Sea).Conn("rum", c.Coast...).Flag(c.Coast...).SC(c.Poland-Lithuania).
 		// arm
 		...
 		// edi
 		Prov("edi").Conn("cly", c.Coast...).Conn("nrg", c.Sea).Conn("nth", c.Sea).Conn("yor", c.Coast...).Conn("lvp", c.Land).Flag(c.Coast...).SC(c.Neutral).
 		// nth
 		...
 		// war
 		Prov("war").Conn("sil", c.Land).Conn("pru", c.Land).Conn("lvn", c.Land).Conn("mos", c.Land).Conn("ukr", c.Land).Conn("gal", c.Land).Flag(c.Land).SC(c.Poland-Lithuania).
 		// ukr
 		...
 		// ank
 		Prov("ank").Conn("con", c.Coast...).Conn("bla", c.Sea).Conn("arm", c.Coast...).Conn("smy", c.Land).Flag(c.Coast...).SC(c.Ottomans).
 		// smy
 		Prov("smy").Conn("aeg", c.Sea).Conn("con", c.Coast...).Conn("ank", c.Land).Conn("arm", c.Land).Conn("syr", c.Coast...).Conn("eas", c.Sea).Flag(c.Coast...).SC(c.Ottomans).
 		// aeg
 		...
 		// gre
 		Prov("gre").Conn("ion", c.Sea).Conn("alb", c.Coast...).Conn("ser", c.Land).Conn("bul", c.Land).Conn("bul/sc", c.Sea).Conn("aeg", c.Sea).Flag(c.Coast...).SC(c.Venice).
 		// nap
 		Prov("nap").Conn("tys", c.Sea).Conn("rom", c.Coast...).Conn("apu", c.Coast...).Conn("ion", c.Sea).Flag(c.Coast...).SC(c.Spain).
 		// tys
 		...
 		// spa
 		Prov("spa").Conn("por", c.Land).Conn("gas", c.Land).Conn("mar", c.Land).Conn("mid", c.Sea).Conn("gol", c.Sea).Conn("wes", c.Sea).Flag(c.Land).SC(c.Spain).
 		// spa/nc
 		Prov("spa/nc").Conn("por", c.Sea).Conn("mid", c.Sea).Conn("gas", c.Sea).Flag(c.Sea).
 		// por
 		Prov("por").Conn("mid", c.Sea).Conn("spa/nc", c.Sea).Conn("spa", c.Land).Conn("spa/sc", c.Sea).Flag(c.Coast...).SC(c.Spain).
 		// gas
 		...
 		// bre
 		Prov("bre").Conn("mid", c.Sea).Conn("eng", c.Sea).Conn("pic", c.Coast...).Conn("par", c.Land).Conn("gas", c.Coast...).Flag(c.Coast...).SC(c.England).
 		// eng
 		...
 		// bul
 		Prov("bul").Conn("ser", c.Land).Conn("rum", c.Land).Conn("con", c.Land).Conn("gre", c.Land).Flag(c.Land).Conn("aeg", c.Sea).Conn("bla", c.Sea).SC(c.Ottomans).
 		// con
 		Prov("con").Conn("bul/sc", c.Sea).Conn("bul", c.Land).Conn("bul/ec", c.Sea).Conn("bla", c.Sea).Conn("ank", c.Coast...).Conn("smy", c.Coast...).Conn("aeg", c.Sea).Flag(c.Coast...).SC(c.Ottomans).
 		// bul/sc
 		...
 		// rom
 		Prov("rom").Conn("tys", c.Sea).Conn("tus", c.Coast...).Conn("ven", c.Land).Conn("apu", c.Land).Conn("nap", c.Coast...).Flag(c.Coast...).SC(c.Neutral).
 		// tus
 		...
 		// bel
 		Prov("bel").Conn("pic", c.Coast...).Conn("eng", c.Sea).Conn("nth", c.Sea).Conn("hol", c.Coast...).Conn("ruh", c.Land).Conn("bur", c.Land).Flag(c.Coast...).SC(c.France).
 		// hol
 		...
 		// mun
 		Prov("mun").Conn("bur", c.Land).Conn("ruh", c.Land).Conn("kie", c.Land).Conn("ber", c.Land).Conn("sil", c.Land).Conn("boh", c.Land).Conn("tyr", c.Land).Flag(c.Land).SC(c.HRE).
 		// boh
 		Prov("boh").Conn("mun", c.Land).Conn("sil", c.Land).Conn("gal", c.Land).Conn("vie", c.Land).Conn("tyr", c.Land).Flag(c.Land).
 		// vie
 		Prov("vie").Conn("tyr", c.Land).Conn("boh", c.Land).Conn("gal", c.Land).Conn("bud", c.Land).Conn("tri", c.Land).Flag(c.Land).SC(c.HRE).
 		// bud
 		Prov("bud").Conn("tri", c.Land).Conn("vie", c.Land).Conn("gal", c.Land).Conn("rum", c.Land).Conn("ser", c.Land).Flag(c.Land).SC(c.Neutral).
 		// tri
 		Prov("tri").Conn("adr", c.Sea).Conn("ven", c.Coast...).Conn("tyr", c.Land).Conn("vie", c.Land).Conn("bud", c.Land).Conn("ser", c.Land).Conn("alb", c.Coast...).Flag(c.Coast...).SC(c.Venice).
 		// ven
 		Prov("ven").Conn("tus", c.Land).Conn("pie", c.Land).Conn("tyr", c.Land).Conn("tri", c.Coast...).Conn("adr", c.Sea).Conn("apu", c.Coast...).Conn("rom", c.Land).Flag(c.Coast...).SC(c.Venice).
 		// pie
 		...
 		// kie
  		Prov("kie").Conn("hol", c.Coast...).Conn("hel", c.Sea).Conn("den", c.Coast...).Conn("bal", c.Sea).Conn("ber", c.Coast...).Conn("mun", c.Land).Conn("ruh", c.Land).Flag(c.Coast...).SC(c.Neutral).
  		Done()
  }
 +
 ...
 +	return graph.New().
 +		// ber
 +		Prov("ber").Conn("lon", c.Land).Conn("par", c.Land).Conn("rom", c.Land).Conn("con", c.Land).Conn("vie", c.Land).Conn("mos", c.Land).Flag(c.Land).SC(c.HRE).
 +		// lon
 ...
 +		// rom
 +		Prov("rom").Conn("ber", c.Land).Conn("lon", c.Land).Conn("par", c.Land).Conn("con", c.Land).Conn("vie", c.Land).Conn("mos", c.Land).Flag(c.Land).SC(c.Neutral).
 +		// con
 +		Prov("con").Conn("ber", c.Land).Conn("lon", c.Land).Conn("par", c.Land).Conn("rom", c.Land).Conn("vie", c.Land).Conn("mos", c.Land).Flag(c.Land).SC(c.Ottomans).
 +		// vie
 +		Prov("vie").Conn("ber", c.Land).Conn("lon", c.Land).Conn("par", c.Land).Conn("rom", c.Land).Conn("con", c.Land).Conn("mos", c.Land).Flag(c.Land).SC(c.HRE).
 +		// mos
 +		Prov("mos").Conn("ber", c.Land).Conn("lon", c.Land).Conn("par", c.Land).Conn("rom", c.Land).Conn("con", c.Land).Conn("vie", c.Land).Flag(c.Land).SC(c.Neutral).
 +		Done()
 +}
